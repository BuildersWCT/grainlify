...
impl BountyEscrowContract {
    ...
    pub fn simulate_lock(
        ...
        if Self::check_paused(&env, symbol_short!("lock")) {
            return SimulationResult {
                success: false,
                error_code: Error::BountyFundsPaused as u32,
                amount: 0,
            ...
            .get::<DataKey, (i128, i128)>(&DataKey::AmountPolicy)
        {
            if amount < min_amount {
                return SimulationResult {
                    success: false,
                    error_code: Error::BountyAmountBelowMinimum as u32,
                    amount: 0,
                    resulting_status: EscrowStatus::Locked,
                    remaining_amount: 0,
                };
            ...
            if amount > max_amount {
                return SimulationResult {
                    success: false,
                    error_code: Error::BountyAmountAboveMaximum as u32,
                    amount: 0,
                    resulting_status: EscrowStatus::Locked,
                    remaining_amount: 0,
                };
    ...
    pub fn simulate_release(env: Env, bounty_id: u64, _contributor: Address) -> SimulationResult {
        ...
        if escrow.status != EscrowStatus::Locked {
            return SimulationResult {
                success: false,
                error_code: Error::BountyFundsNotLocked as u32,
                amount: 0,
    ...
    pub fn simulate_refund(env: Env, bounty_id: u64) -> SimulationResult {
            ...
            .has(&DataKey::PendingClaim(bounty_id))
        {
            ...
            if !claim.claimed {
                return SimulationResult {
                    success: false,
                    error_code: Error::BountyClaimPending as u32,
                    amount: 0,
                    resulting_status: escrow.status,
                    remaining_amount: escrow.remaining_amount,
                };
