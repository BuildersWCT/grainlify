...
fn test_simulate_lock_paused() {
    ...
    assert!(!result.success);
    assert_eq!(result.error_code, Error::BountyFundsPaused as u32);
}
...
fn test_simulate_release_already_released() {
    ...
    assert!(!result.success);
    assert_eq!(result.error_code, Error::BountyFundsNotLocked as u32);
}
...
fn test_simulate_refund_with_pending_claim_fails() {
    ...
    assert!(!result.success);
    assert_eq!(result.error_code, Error::BountyClaimPending as u32);
}
