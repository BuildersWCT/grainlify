...
describe('Min/Max Amount Policy Enforcement', () => {
  ...
  describe('parseContractErrorByCode – bounty escrow numeric codes', () => {
    it('maps numeric code 208 to AMOUNT_BELOW_MIN for bounty_escrow', () => {
      const error = parseContractErrorByCode(208, 'bounty_escrow');
      
      expect(error).toBeInstanceOf(ContractError);
      expect(error.code).toBe(ContractErrorCode.AMOUNT_BELOW_MIN);
      expect(error.contractErrorCode).toBe(208);
      expect(error.message).toContain('below');
    });

    it('maps numeric code 209 to AMOUNT_ABOVE_MAX for bounty_escrow', () => {
      const error = parseContractErrorByCode(209, 'bounty_escrow');
      
      expect(error).toBeInstanceOf(ContractError);
      expect(error.code).toBe(ContractErrorCode.AMOUNT_ABOVE_MAX);
      expect(error.contractErrorCode).toBe(209);
      expect(error.message).toMatch(/exceeds|maximum/i);
    });

    it('returns user-friendly message for code 208', () => {
      const error = parseContractErrorByCode(208, 'bounty_escrow');
      expect(error.message).toMatch(/minimum.*policy/i);
    });

    it('returns user-friendly message for code 209', () => {
      const error = parseContractErrorByCode(209, 'bounty_escrow');
